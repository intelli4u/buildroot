...
AUTORECONF = $(HOST_CONFIGURE_OPTS) ACLOCAL="$(ACLOCAL)" AUTOCONF="$(AUTOCONF)" AUTOHEADER="$(AUTOHEADER)" AUTOMAKE="$(AUTOMAKE)" AUTOPOINT=/bin/true $(HOST_DIR)/bin/autoreconf -f -i $(if $(wildcard $(ACLOCAL_DIR)),-I "$(ACLOCAL_DIR)") $(if $(wildcard $(ACLOCAL_HOST_DIR)),-I "$(ACLOCAL_HOST_DIR)")
